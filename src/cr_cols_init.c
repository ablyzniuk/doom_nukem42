...
/*   Updated: 2019/06/05 13:02:50 by yalytvyn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_get_wall(t_main *m, t_cols_wall *cols, size_t wall_sect, size_t sect)
{
	size_t	k;
...
void	ft_add_cols_next(t_main *m, size_t old_sect, size_t sect)
{
	...
	while (wall_sect < m->sector[sect].vertex_arr_len)
	{
		if (m->sector[sect].transit[wall_sect] == -1)
		{
			...
			ft_add_cols_next_next(m, old_sect, m->sector[sect].transit[wall_sect]);
		wall_sect++;
	}
}
