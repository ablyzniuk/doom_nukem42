...
/*   Updated: 2019/06/27 00:03:09 by ablizniu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_load_texture(t_main *m)
{
	...
	if (!(m->texture[5] = SDL_LoadBMP("./resources/en/text/6.bmp")))
		exit(-11);
	if (!(m->texture[6] = SDL_LoadBMP("./resources/en/text/7.bmp")))
		exit(-11);
	if (!(m->sprite.ch = SDL_LoadBMP("./resources/en/ch/chr.bmp")))
		exit(-11);
	if (!(m->pict.end[0] = SDL_LoadBMP("./resources/en/scrns/end.bmp")) ||
	!(m->pict.start = SDL_LoadBMP("./resources/en/scrns/start.bmp")))
		exit(3);
	if (!(m->pict.end[1] = SDL_LoadBMP("./resources/en/scrns/end1.bmp")))
		exit(3);
}
...
void			ft_load_music(t_main *m)
{
	...
	m->music.chanel = Mix_PlayMusic(m->music.audio, 10);
	Mix_PauseMusic();
}
